...
class TestPackStats(unittest.TestCase):
    ...
    def test_lists_architectures(self):
        ...
        result = get_content_files_list(self.mirror_url)
    ...
    def test_get_content_index_file_url(self):
        """the application can get the content index file url given the architecture
        and whether or not udeb files are needed"""
        from packstats.packstats import get_contents_file_urls

        urls = get_contents_file_urls(mirror_url=self.mirror_url, arch=self.architecture)
    ...
    def test_downloads_content_file(self):
        ...
        urls = get_contents_file_urls(self.architecture, self.mirror_url)
    ...
    def test_gets_package_data_from_contents(self):
        ...
        urls = get_contents_file_urls(self.architecture, self.mirror_url, include_udeb=True)
        with tempfile.TemporaryDirectory() as temp_directory:
            # merge udeb and deb info, if required.
            complete_package_data = {}
            total_keys = 0
            for url in urls:
                contents_file = download_contents_file(url, output_dir=temp_directory)
