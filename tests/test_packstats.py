    ...
    def test_gets_package_data_from_contents(self):
        ...
        with tempfile.TemporaryDirectory() as temp_directory:
            ...
            for url in urls:
                ...
                self.assertIsInstance(package_dict, dict, "Package metadata should be a dictionary")
                self.assertTrue(len(package_dict.keys()) > 0, "Package list is not empty")
                complete_package_data.update(**package_dict)
                total_keys += len(package_dict)
            self.assertEqual(total_keys, len(complete_package_data), "the two package files were not added together")
